...
library(shiny)
exped <- exped_tidy %>% 
  select("EXPID", "PEAKID", "YEAR", "SEASON_FACTOR", "NATION", "TOTDAYS", "TERMREASON_FACTOR", "TOTMEMBERS", "MDEATHS", "TOTHIRED", "O2USED") %>% 
  mutate(SUCCESS = TERMREASON_FACTOR == "Success (main peak)")

nation_summary <- exped %>% 
  group_by(NATION, SEASON_FACTOR) %>% 
  summarize(
    TOTSUCCESS = sum(SUCCESS == TRUE, na.rm = TRUE),
    MEAN_TOTMEMBERS = mean(TOTMEMBERS, na.rm = TRUE),
    MEAN_PCT_O2 = mean(O2USED == TRUE, na.rm = TRUE) * 100,
    .groups = 'drop'
  ) %>% 
  group_by(NATION) %>% 
  mutate(TOTSUCCESS_NATION = sum(TOTSUCCESS)) %>% 
  ungroup()

nation_summary <- nation_summary %>%
  mutate(NATION = factor(NATION, levels = unique(NATION[order(TOTSUCCESS_NATION)]))) %>% 
  arrange(desc(TOTSUCCESS_NATION)) %>%
  head(39)
...
ui <- fluidPage(
  ...
  h1("Successful Expeditions by Nation and Season"),
  fluidRow(
    column(
      3,
      div(
        class = "sidebar",
        h5("Select this checkbox to view the successes for each country by the season that they were done in:"),
        checkboxInput("display_seasons", "Display Seasons", value = FALSE),
      )
    ),
    column(
      9,
...
server <- function(input, output) {
  output$success_plot <- renderPlot({
    plot_data <- if (input$display_seasons) {
      nation_summary
    } else {
      nation_summary %>% 
        group_by(NATION) %>% 
        summarize(TOTSUCCESS = sum(TOTSUCCESS), .groups = 'drop') %>%
        mutate(SEASON_FACTOR = "Total")
    }
    
    color_palette <- if (input$display_seasons) {
      c("Winter" = "#2b6cb0", "Spring" = "#38a169", "Summer" = "#f6ad55", "Autumn" = "#e53e3e", "Total" = "#805ad5")
    } else {
      colorRampPalette(c("#4E79A7", "#F28E2B", "#E15759", "#76B7B2", "#59A14F"))(length(unique(plot_data$NATION)))
    }
    
    ggplot(plot_data) +
      geom_col(aes(x = TOTSUCCESS, y = NATION, fill = if (input$display_seasons) SEASON_FACTOR else NATION), position = "dodge") +
      scale_fill_manual(values = if (input$display_seasons) color_palette else setNames(color_palette, unique(plot_data$NATION)),
                        guide = if (input$display_seasons) guide_legend() else "none") +
      labs(x = "Total Successful Expeditions",
           y = "Nation",
           fill = if (input$display_seasons) "Season" else NULL) +
      theme_minimal()
  })
}
